...
class User {
  ...
  Future<bool> signupWithEmailAndPwd(
      String name, String email, String pwd) async {
    bool response =
        await _firebaseController.signUpWithEmailAndPassword(email, pwd);
    if (response == true) {
  ...
  Future<bool> getUserFromDB(email) async {
    ...
    bool response = await _firebaseController.getUserFromDB(
      _userDetails,
      email,
    );
  ...
  Future<bool> addUserToDB(File image) async {
    bool response = await _firebaseController.addUserToDb(
      _userDetails,
      image,
    );
    return response;
  ...
  getUserDescriptor() => _userDetails;
}
