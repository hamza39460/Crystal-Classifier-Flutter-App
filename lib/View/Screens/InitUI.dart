    ...
    return Scaffold(
      body: ChangeNotifierProvider.value(
        ...
        child: Consumer<WorkSpaceController>(
            builder: (context, workSpaceController, child) {
          debugPrint(
              "Workspace state ${workSpaceController.getWorkspaceCurrentState()}");
          debugPrint(
              "Data state ${Provider.of<UserController>(context).getUserDataState()}");
          switch (workSpaceController.getWorkspaceCurrentState()) {
            case WorkspaceState.Uninitialized:
              {
                if (Provider.of<UserController>(context).getUserDataState() ==
                    UserDataState.Fetched_User_Data)
                  Future(() {
                    log('hello');
                    workSpaceController.fetchAllWorkspaces();
                  });
